...
{
    public class DealsDashboardBase : ComponentBase
    {
        ...
        [Inject] protected ILogger<DealsDashboardBase> Logger { get; set; }
        ...
        protected async Task LoadSubscriptionDetailsAsync(int verticalId)
        {
            ...
            {
                new() { Title = "Published Deals", Value = $"{response.ItemsDashboard.PublishedAds}", Icon = "PublishedDeals.svg" },
                ...
                new() { Title = "Views", Value = $"{response.ItemsDashboard.Views:N0}", Icon = "Views.svg" },
                   new() { Title = "Downloads", Value = $"{response.ItemsDashboard.WhatsAppClicks}", Icon = "Download_Statistics.svg" },
        ...
        protected async void onRemove(string adId)
        {
            try
            {
                var result = await ClassfiedDashboardService.RemoveDealsAdAsync(adId);
                if (result)
                {
        ...
        protected void NavigateToCreateProfile()
        {
            ...
            var categoryId = SubVerticalId.Deals;
