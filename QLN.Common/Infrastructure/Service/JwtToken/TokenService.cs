    ...
    public class TokenService : ITokenService
    {
        ...
        public async Task<string> GenerateEnrichedAccessToken(ApplicationUser user, DrupalUser drupalUser, DateTime expiry, IList<string>? roles)
        {
            ...
            var authClaims = new List<Claim>
            {
                ...
                new(JwtRegisteredClaimNames.PhoneNumber, user.PhoneNumber ?? user.PhoneNumber?? string.Empty),
                new(JwtRegisteredClaimNames.Name, drupalUser.Name ?? user.FirstName?? string.Empty),
            ...
            // Due to token weightage concerns, we are not adding the entire subscriptions/companies as claims and added that data to response.
