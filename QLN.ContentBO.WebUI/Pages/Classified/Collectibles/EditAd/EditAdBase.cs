...
namespace QLN.ContentBO.WebUI.Pages.Classified.Collectibles.EditAd
{
    public class EditAdBase : ComponentBase
    {
        ...
        protected CollectiblesEditAdPost adPostModel { get; set; } = new();
        ...
        private async Task LoadAdDataAsync()
        {
            try
            {
                ...
                if (response is { IsSuccessStatusCode: true })
                {
                    ...
                    adPostModel = JsonSerializer.Deserialize<CollectiblesEditAdPost>(json, new JsonSerializerOptions
                    {
                        PropertyNameCaseInsensitive = true
                    }) ?? new CollectiblesEditAdPost();
        ...
        private async Task UpdateAdToApiAsync()
        {
            try
            {
                ...
                var response = await ClassifiedService.UpdateAdAsync("collectibles", payload);
       ...
       private async Task<string?> UploadCertificateAsync()
        ...
        private async Task LoadCategoryTreesAsync()
        {
            try
            {
                var response = await ClassifiedService.GetServicesCategories(Vertical.Classifieds, SubVertical.Collectibles);
