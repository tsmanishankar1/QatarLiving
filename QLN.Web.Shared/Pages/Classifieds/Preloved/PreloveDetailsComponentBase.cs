...
{
    public class PreloveDetailsComponentBase : ComponentBase
    {
        [Inject] private IClassifiedsServices _classifiedsService { get; set; } = default!;
        [Inject] protected ILogger<PreloveDetailsComponentBase> Logger { get; set; }
        ...
        protected ClassifiedsIndex PrelovedsDetails { get; set; } = new();
        ...
        protected override async Task OnInitializedAsync()
        {
            try
            {
                ...
                {
                    PrelovedsDetails = await response.Content.ReadFromJsonAsync<ClassifiedsIndex>();
                }
                else
                {
