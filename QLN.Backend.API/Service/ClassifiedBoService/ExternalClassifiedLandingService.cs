...
namespace QLN.Backend.API.Service.V2ClassifiedBoService
{
    public class ExternalClassifiedLandingService : IClassifiedBoLandingService
    {
        ...
        public ExternalClassifiedLandingService(
            ...
            ISearchService searchService, IV2SubscriptionService subscriptionService, IClassifiedService classifiedService)
        {
        ...
        public async Task<string> CreateFeaturedCategory(string userId, string userName, FeaturedCategoryDto dto, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                catch (JsonException jsonEx)
                {
                    ...
                    throw new Exception($"Unexpected response format: {rawJson}");
                }
            }
        ...
        public async Task<List<FeaturedCategory>> GetFeaturedCategoriesByVertical(Vertical vertical, CancellationToken cancellationToken = default)
        {
            try
            {
                var trees = await _dapr.InvokeMethodAsync<List<FeaturedCategory>>(
                    HttpMethod.Get,
                    SERVICE_APP_ID,
                    $"/api/v2/classifiedbo/getfeaturedcategoriesbyvertical?vertical={vertical}",
                    cancellationToken);
            ...
            catch (InvocationException ex)
            {
                ...
                throw new Exception($"Failed to get L1 categories for {vertical}", ex);
            }
        }

        public async Task<string> ReorderFeaturedCategorySlots(string userId, string userName, LandingBoSlotReorderRequest dto, CancellationToken cancellationToken = default)
        {
            try
            {
                var response = await _dapr.InvokeMethodAsync<LandingBoSlotReorderRequest, string>(
                    HttpMethod.Put,
                    SERVICE_APP_ID,
                    $"/api/v2/classifiedbo/reorderfeaturedcategoryslots?userid={userId}&username={userName}",
                    dto,
                    cancellationToken);
        ...
        public async Task<string> ReplaceFeaturedCategorySlots(string userId, string userName, LandingBoSlotReplaceRequest dto, CancellationToken cancellationToken = default)
        {
            try
            {
                var response = await _dapr.InvokeMethodAsync<LandingBoSlotReplaceRequest, string>(
                    HttpMethod.Put,
                    SERVICE_APP_ID,
                    $"/api/v2/classifiedbo/replace-slot?userid={userId}&username={userName}",
                    dto,
                    cancellationToken);
        ...
        public async Task<string> DeleteFeaturedCategory(string id, Vertical vertical, string userId, string userName, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                var queryParams = $"?vertical={vertical}&categoryId={id}&userid={userId}&username={userName}";
        ...
        public async Task<FeaturedCategory> GetFeaturedCategoryById(string id, CancellationToken cancellationToken = default)
        {
            try
            {
                var response = await _dapr.InvokeMethodAsync<FeaturedCategory>(
                    HttpMethod.Get,
                    SERVICE_APP_ID,
                    $"/api/v2/classifiedbo/getfeaturedcategory?id={id}",
                    cancellationToken);
                if (response == null)
                    throw new KeyNotFoundException("FeaturedCategory not found");
        ...
        public async Task<string> EditFeaturedCategory(string userId, string userName, EditFeaturedCategoryDto dto, CancellationToken cancellationToken = default)
        {
            try
            {
                var response = await _dapr.InvokeMethodAsync<EditFeaturedCategoryDto, string>(
                    HttpMethod.Put,
                    SERVICE_APP_ID,
                    $"/api/v2/classifiedbo/edit-featured-category?userId={userId}&userName={userName}",
                    dto,
                    cancellationToken);
                if (response == null)
    ...
    CancellationToken cancellationToken = default)
        ...
        private string ExtractErrorMessage(string errorJson)
        {
            ...
            {
                var problem = JsonSerializer.Deserialize<ProblemDetails>(errorJson);
                return problem?.Detail ?? "Unknown validation error.";
            }
            catch
            {
                return errorJson;
    ...
    CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                // Fetch all collectible ads by IDs
                var ads = await Task.WhenAll(
                    request.AdIds.Select(id => _classifiedService.GetCollectiblesAdById(id, cancellationToken))
                );

                // Group by subscription and action
                var groupedActions = ads
                    .Where(a => a != null && a.SubscriptionId.HasValue && a.SubscriptionId.Value != Guid.Empty)
                    .GroupBy(a => new
                    {
                ...
                foreach (var group in groupedActions)
                {
                    ...
                    // Validate subscription quota before making internal call
                    var canUse = await _subscriptionContext.ValidateSubscriptionUsageAsync(
                        subscriptionId,
                        actionName,
                        usageCount,
                    ...
                    if (!canUse)
                    {
                        failReason = $"Insufficient subscription quota for {actionName}";
                        failedIds.AddRange(group.Select(x => x.Id));
                        continue; // move to next subscription group
                    }

                    // Call internal service for this subscription group
                    var firstSubId = subscriptionId;
                    var url = $"api/v2/classifiedbo/bulk-collectibles-action-userid/{userId}&subscriptionId={firstSubId}";
                    var serviceRequest = _dapr.CreateInvokeMethodRequest(HttpMethod.Post, SERVICE_APP_ID, url);
                    serviceRequest.Content = new StringContent(JsonSerializer.Serialize(request), Encoding.UTF8, "application/json");
                    ...
                    {
                        var errorJson = await response.Content.ReadAsStringAsync(cancellationToken);
                        failReason = ExtractErrorMessage(errorJson);
                        failedIds.AddRange(group.Select(x => x.Id));
                        continue;
                    }

                    // Record quota usage
                    var actionDisplay = group.Key.ActionType switch
                    {
                        BulkActionEnum.Promote => "Promote",
                        BulkActionEnum.Feature => "Feature",
                        BulkActionEnum.UnPromote => "unpromote",
                    ...
                    {
                        failReason = $"Failed to record usage for {actionDisplay}";
                        failedIds.AddRange(group.Select(x => x.Id));
                    }
                }

                // Final result
                var result = new BulkAdActionResponseitems
                {
                    Failed = new ResultGroup
                    {
                        Count = failedIds.Count,
            ...
            {
                _logger.LogError(ex, "Error in BulkCollectiblesAction");
                throw new InvalidOperationException("Bulk collectibles action failed due to an unexpected error.", ex);
            }
        }
