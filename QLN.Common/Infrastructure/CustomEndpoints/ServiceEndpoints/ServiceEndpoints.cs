...
namespace QLN.Common.Infrastructure.CustomEndpoints.ServiceEndpoints
{
    public static class ServiceEndpoints
    {
        ...
        public static RouteGroupBuilder MapClassifiedsCountEndpoints(this RouteGroupBuilder group)
        {
            // GET endpoint - uses token and query param
            group.MapGet("/getsubverticalstatuscounts", async Task<Results<Ok<ServicesStatusCountsDto>, ProblemHttpResult>> (
                HttpContext httpContext,
            ...
            ) =>
            {
                try
                {
                    ...
                    if (string.IsNullOrWhiteSpace(subVerticalName))
                    {
                        ...
                        {
                            Title = "Bad Request",
                            Detail = "SubVerticalName is required.",
                            Status = StatusCodes.Status400BadRequest
                        });
                    }

                    var counts = await service.GetSubverticalStatusCountsAsync(uid, subVerticalName, cancellationToken);
                    return TypedResults.Ok(counts);
                }
                catch (Exception ex)
                {
            ...
            .WithName("GetSubverticalStatusCounts")
            .WithTags("Service")
            .WithSummary("Get subvertical status counts")
            .WithDescription("Returns counts of published, promoted, and featured services for the given subvertical and logged-in user.")
            .Produces<ServicesStatusCountsDto>(StatusCodes.Status200OK)
            .Produces<ProblemDetails>(StatusCodes.Status400BadRequest)
            .Produces<ProblemDetails>(StatusCodes.Status403Forbidden)
            .Produces<ProblemDetails>(StatusCodes.Status500InternalServerError);

            // POST endpoint - used internally
            group.MapPost("/getsubverticalstatuscountsbyuserid", async Task<Results<Ok<ServicesStatusCountsDto>, ProblemHttpResult>> (
                [FromBody] SubverticalStatusRequest req,
                [FromServices] IServices service,
                CancellationToken cancellationToken
            ) =>
            {
                try
                {
                    var counts = await service.GetSubverticalStatusCountsAsync(req.UserId, req.SubVerticalName, cancellationToken);
                    return TypedResults.Ok(counts);
                }
                catch (Exception ex)
                {
            ...
            .ExcludeFromDescription()
            .WithName("GetSubverticalStatusCountsByUserId")
            .WithTags("Service");
