...
namespace QLN.Web.Shared.Components.NewCustomSelect
{
...

    {
       ...
       [Parameter] public EventCallback<bool> OnFreeToggleChanged { get; set; }
        ...
        [Parameter] public List<EventCategoryV2> Categories { get; set; } = [];
        [Parameter] public EventCallback<(string from, string to)> OnDateChanged { get; set; }
        [Parameter] public List<LocationDto.AreaDto> Areas { get; set; } = [];
        protected List<LocationDto.AreaDto> FilteredAreas = new(); // filtered list for search
        protected List<LocationDto.AreaDto> SelectedAreas = new(); // selected areas
       ...
       protected bool ShouldShowClearAll =>
        ...
        protected DotNetObjectReference<SearchSectionBaseV2>? _dotNetRef;
     ...
     protected async Task HandleSearchChange(ChangeEventArgs e)
    {
       ...
       protected async Task HandleLocationSelectionChanged(List<LocationDto.AreaDto> selected)
        ...
        protected async Task CallApiForLocationChange(List<LocationDto.AreaDto> selected)
        {
